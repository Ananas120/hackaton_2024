...
@execute_eagerly(signature = eeg_signature, numpy = True)
def remove_environmental_noise(eeg, rate, noise_type = NoiseTypes.FIFTY_AND_SIXTY, ** _):
...
@execute_eagerly(signature = eeg_signature, numpy = True)
def reduce_eeg_noise(eeg, rate, channels, channels_to_denoise = ['Fp1'], wave_type = WaveletTypes.COIF3, decomposition_level = 1, ** _):
    if isinstance(eeg, tf.Tensor): eeg = eeg.numpy()
    eeg = eeg.astype(np.float64)
